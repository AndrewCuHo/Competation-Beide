...
from models import EfficientNet, resnext50_32x4d_wsl
...
def Resnext50_32x4d(num_classes, test=False):
    model = resnext50_32x4d_wsl()
    if not test:
        if LOCAL_PRETRAINED['resnext50_32x4d'] == None:
            state_dict = load_state_dict_from_url(model_urls['resnext50_32x4d'], progress=True)
        else:
            state_dict = torch.load(LOCAL_PRETRAINED['resnext50_32x4d'])
        model.load_state_dict(state_dict)
    fc_features = model.fc.in_features
    model.fc = nn.Linear(fc_features, num_classes)
    return model
